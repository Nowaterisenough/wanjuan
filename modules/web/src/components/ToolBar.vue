...
const saveSource = () => {
  ...
  if (isInvaildSource(source)) {
    ...
    API.saveSource(source).then(({ data }) => {
      ...
      if (data.isSuccess) {
        ElMessage({
          message: `源《${sourceName}》已成功保存到「万卷 3.0 App」`,
          type: 'success',
        })
        //save to store
...
const saveHotkeysConfig = (config: string[][]) => {
  localStorage.setItem('wanjuan_web_hotkeys', JSON.stringify(config))
}
...
function readHotkeysConfig() {
  try {
    const localStorageConfig = localStorage.getItem('wanjuan_web_hotkeys')
    ...
    localStorage.removeItem('wanjuan_web_hotkeys')
  }
  return false
}
