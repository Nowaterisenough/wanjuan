...
export const parseLeagdoHttpUrlWithDefault = (
  ...
  let wanjuan_webSocket_port
  if (port !== '') {
    wanjuan_webSocket_port = String(Number(port) + 1)
  } else {
    wanjuan_webSocket_port = protocol.startsWith('https:') ? '444' : '81'
  }
  // websocket协议是否为加密版本
  const wanjuan_webSocket_protocol = protocol.startsWith('https:')
    ? 'wss://'
    : 'ws://'

  const http_entry_point = url.toString()

  url.protocol = wanjuan_webSocket_protocol
  url.port = wanjuan_webSocket_port
  const webSocket_entry_point = url.toString()

  console.info('wanjuan_api_config:')
  console.table({
