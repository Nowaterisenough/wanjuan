/** https://github.com/gedoor/wanjuan/tree/master/app/src/main/java/io/wanjuan/app/api */
/** https://github.com/gedoor/wanjuan/tree/master/app/src/main/java/io/wanjuan/app/web */
...
export let wanjuan_http_entry_point = ''
export let wanjuan_webSocket_entry_point = ''
...
export const setApiEntryPoint = (
  ...
  wanjuan_http_entry_point = new URL(http_entry_point).toString()
  wanjuan_webSocket_entry_point = new URL(webSocket_entry_point).toString()
  ajax.defaults.baseURL = wanjuan_http_entry_point
}

// 书架API
// Http
const getReadConfig = async (http_url = wanjuan_http_entry_point) => {
...
const saveBookProgressWithBeacon = (bookProgress: BookProgress) => {
  ...
  navigator.sendBeacon(
    new URL('saveBookProgress', wanjuan_http_entry_point),
...
const search = (
  ...
  const socket = new WebSocket(
    new URL('searchBook', wanjuan_webSocket_entry_point),
  )
...
const getProxyCoverUrl = (coverUrl: string) => {
  if (coverUrl.startsWith(wanjuan_http_entry_point)) return coverUrl
  return new URL(
    'cover?path=' + encodeURIComponent(coverUrl),
    wanjuan_http_entry_point,
  ).toString()
}
/**
 * 从阅读获取需要特定处理的图片
...
const getProxyImageUrl = (
  ...
  if (src.startsWith(wanjuan_http_entry_point)) return src
  return new URL(
    'image?path=' +
