...
import io.wanjuan.app.help.CoverThumbnailCache
import io.wanjuan.app.help.config.AppConfig
import io.wanjuan.app.help.config.CoverCollectionManager
import io.wanjuan.app.help.config.CoverCollectionManager.isRealCoverPath
...
import io.wanjuan.app.utils.textHeight
...
class CoverImageView @JvmOverloads constructor(
    ...
    private fun findSourceOrigin(path: String?, name: String?, author: String?): String? {
        if (!path.isNullOrBlank()) {
            runCatching {
                io.wanjuan.app.data.appDb.bookDao.getBookByCover(path)?.origin
            }.getOrNull()?.let { return it }
            runCatching {
                io.wanjuan.app.data.appDb.searchBookDao.getSearchBookByCover(path)?.origin
            }.getOrNull()?.let { return it }
        }
        if (!name.isNullOrBlank() && !author.isNullOrBlank()) {
            runCatching {
                io.wanjuan.app.data.appDb.bookDao.getBook(name, author)?.origin
            }.getOrNull()?.let { return it }
        }
        return null
    }
