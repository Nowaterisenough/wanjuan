package io.wanjuan.app.help.ai
    ...
    private fun createSourceDefinition(): JSONObject {
        return JSONObject().apply {
            ...
            put("function", JSONObject().apply {
                ...
                put(
                    "description",
                    "创建、规范化或预览 Wanjuan 书源草稿。可传完整 sourceJson，也可传基础字段和规则字段。创建后必须继续调用 debug_book_source 调试；调试失败时应调用 update_book_source 修改草稿，再继续调试。只有用户明确要求保存时才 save=true 写入本地书源库。"
                )
    ...
    private fun getSourceDefinition(): JSONObject {
        return JSONObject().apply {
            ...
            put("function", JSONObject().apply {
                ...
                put(
                    "description",
                    "读取本地已保存的 Wanjuan 书源。bookSourceUrl 精确读取；searchKey 可按名称、分组、URL、注释搜索。用于修改已有书源前获取当前完整 JSON。"
                )
                put("parameters", JSONObject().apply {
                    put("type", "object")
                    put("properties", JSONObject().apply {
    ...
    private fun updateSourceDefinition(): JSONObject {
        return JSONObject().apply {
            ...
            put("function", JSONObject().apply {
                ...
                put(
                    "description",
                    "修改 Wanjuan 书源草稿或本地已保存书源。可传 sourceJson 作为草稿基底，或传 bookSourceUrl 读取本地书源；推荐用 patch 做深度合并，也可直接传 bookSourceName/searchUrl/ruleToc/ruleContent 等顶层字段作为修改项。patch 字段值为 null 表示清空。若只传 sourceJson 和 save=true，则直接保存该完整书源。返回修改后的完整 sourceJson。调试失败后用本工具修正，再调用 debug_book_source。"
                )
                put("parameters", JSONObject().apply {
                    put("type", "object")
                    put("properties", JSONObject().apply {
    ...
    private fun fetchHtmlDefinition(): JSONObject {
        return JSONObject().apply {
            ...
            put("function", JSONObject().apply {
                ...
                put(
                    "description",
                    "按 Wanjuan 的 AnalyzeUrl/书源配置真实获取网页 HTML，用于书源 agent 分析搜索页、详情页、目录页、正文页。可传 sourceJson 或 bookSourceUrl 复用书源 header/cookie/webView 配置；返回状态码、最终 URL、HTML 片段。"
                )
                put("parameters", JSONObject().apply {
                    put("type", "object")
                    put("properties", JSONObject().apply {
                        put("url", stringProp("要获取的搜索页、详情页、目录页或正文页 URL。支持 Wanjuan URL 规则。"))
    ...
    private fun debugSourceDefinition(): JSONObject {
        return JSONObject().apply {
            ...
            put("function", JSONObject().apply {
                ...
                put(
                    "description",
                    "使用 Wanjuan 原生 Debug 流程调试书源，支持搜索、详情 URL、发现、目录和正文。返回调试日志。若 success=false，必须根据 logs 修改 sourceJson，然后再次调用 create_book_source(save=false) 和 debug_book_source，最多重试 3 轮。"
                )
                put("parameters", JSONObject().apply {
                    put("type", "object")
                    put("properties", JSONObject().apply {
