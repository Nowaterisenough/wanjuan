...
import io.wanjuan.app.help.coroutine.ActivelyCancelException
...
object ParagraphRuleProcessor {
    ...
    suspend fun debug(rule: ParagraphRule, book: Book, chapter: BookChapter, content: String): DebugResult {
        ...
        val processed = withTimeout(rule.validTimeout()) {
            logs.add("paragraph debug start: ${book.name} / ${chapter.title}")
            applyRule(rule, book, chapter, normalizedContent, currentCoroutineContext()).also {
                logs.add("paragraph debug finish")
            }
        }
