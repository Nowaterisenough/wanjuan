...
class BookSourceConfigTest {
    ...
    fun missavActressListStaysInSettingsAndLoadsMoreFromSelectScroll() {
        ...
        assertTrue("MissAV actress WebView fetch should wait for a populated actress list, not the first partial link", exploreUrl.contains("matches.length >= 6"))
        ...
        assertTrue("MissAV should self-heal partial actress caches by reloading page one", exploreUrl.contains("Object.keys(actressMap).length < 6"))
        ...
        assertTrue("MissAV settings should expose a visible actress refresh button", exploreUrl.contains("const actressRefreshKey = '刷新女优列表'"))
        ...
        assertTrue("MissAV actress refresh button should clear stale actress cache before reloading page one", exploreUrl.contains("setInfo('\" + actressCacheKey + \"', '{}')"))
        assertTrue("MissAV actress refresh button should clear stale route cache before reloading page one", exploreUrl.contains("setInfo('\" + actressRouteCacheKey + \"', '{}')"))
        ...
        assertTrue("MissAV actress refresh button should request more actresses and refresh discovery", exploreUrl.contains("java.refreshExplore"))
        ...
        assertTrue("MissAV actress load-more should only set the next-page marker and let discovery refresh the settings rows", exploreUrl.contains("setInfo('\" + actressNeedMoreKey + \"', '1');"))
